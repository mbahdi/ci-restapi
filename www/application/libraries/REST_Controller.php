...
abstract class REST_Controller extends CI_Controller {
    ...
    */
    ...
    public function __construct($config = 'rest')
    {
        ...
        // auth app and login
        // if (count($this->uri->segments) == 2 && $this->uri->segment(2) !== 'login') {
        //     $this->_check_auth_app();
        // }
        $this->_check_auth_app();
    }
    ...
    /**
     * Checks auth app and login
     *
     * @access protected
     * @return void
     */
    protected function _check_auth_app()
    {
        // print_r($this->_head_args);
        $resp = array();

        if ($this->head('Token')) {
            $token  = JWT::decode($this->head('Token'), $this->config->item('jwt_key'));            
            if ($token) {
                $this->_check_uagent($token->agent);


                //expired token
                if ($this->config->item('exp_token') && (time() - $token->tokentime) > $this->config->item('exp_token')) {
                    $resp = array(
                        'code'=>$this->config->item('retoken_code'),
                        'message'=>$this->config->item('retoken_msg')
                    );
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST' || $_SERVER['REQUEST_METHOD'] === 'PUT' || $_SERVER['REQUEST_METHOD'] === 'PATCH') {
                    ...
                    $isAllowUri = $this->_check_allowed_uris();
     ...
     * @return void
     */
    protected function _check_uagent($uagent)
    {
        // print_r($this->_head_args);
        // print_r($this->head());
        // echo($uagent."<=>".$this->head('User-Agent')."<br/>");
        if ($uagent !== $this->head('User-Agent')) 
        {
            $resp = array(
                'code'=>$this->config->item('uadenide_code'),
                'message'=>$this->config->item('uadenide_msg')
            );
    ...
    /*
    * Check allowed urls without login
    */
    protected function _check_allowed_uris()
    {
        $tSegment   = $this->uri->total_segments();
        $return     = 0;

        // $this->uri->uri_string()
        // $this->uri->total_segments()

        $uriAllowNoLogin = array(
            "categories",
            "post",
            "photos"
            // "users"
        ...
        if ($tSegment == 2) {
            $return = ($this->uri->segment(2) == 'login' ? 1 : $return);
        }

        if ($tSegment == 1) {
            $return = (in_array($this->uri->segment(1), $uriAllowNoLogin) ? 1 : $return);
        }
    ...
    */
    protected function _check_logged_in()
    {
        if (count($this->uri->segments) == 2 && $this->uri->segment(2) == 'login') {
            ...
            if ($this->head('Auth')) {
                $auth  = JWT::decode($this->head('Auth'), $this->config->item('jwt_key'));

                if ($auth) {
                    $this->_check_uagent($auth->agent);

                    //expired login
                    // if ($this->config->item('exp_login') && (time() - $auth->usertime) > $this->config->item('exp_token')) {

                    //     $resp = array(
                    //         'code'=>$this->config->item('relogin_code'),
                    //         'message'=>$this->config->item('relogin_msg')
                    //     );
    ...
    * Build query GET
    */
    protected function _build_query_get($id=null)
    {
        $this->page         = ($this->get('p') ? $this->get('p') : 1);
        $this->limit        = ($this->get('limit') ? $this->get('limit') : 100);
        $this->offset       = ($this->page-1) * $this->limit;
        $this->search       = ($this->get('s') ? $this->get('s') : NULL);
        $this->sort         = ($this->get('sort') ? $this->get('sort') : NULL);
        $this->view         = ($this->get('view') ? $this->get('view') : NULL);

        $qdata              = new stdClass();        
        $qdata->limit       = $this->limit;
        $qdata->offset      = ($this->page-1) * $this->limit;
        ...
        if (!isset($where['trash']) || isset($where['trash >']) || isset($where['trash <']) || isset($where['trash !=']) || isset($where['trash >=']) || isset($where['trash <='])) {
            $where['trash'] = 0;
        }
        ...
        $qdata->where = $where;
        $qdata->like = (count($like) > 0 ? $like : null);
    ...
    * Build query POST
    */
    protected function _build_query_post()
    {
    ...
    * Build return
    */
    protected function _build_return($dt=null) {
        ...
        $rdt->total     = 0;
        $rdt->limit     = intval($this->limit);
        $rdt->offset    = $this->offset;
        $rdt->page      = intval($this->page);
        $rdt->data      = array();
    ...
    protected function _build_return_products($dt=null) {
        // $page       = ($this->get('p') ? $this->get('p') : 1);
        // $limit      = ($this->get('limit') ? $this->get('limit') : 100);
        // $search     = $this->get('s');
        // $sort       = $this->get('sort');
        ...
        if ($dt && count($dt) > 0) {
            ...
            foreach ($dt as $key => $value) {
                $rdt->subtotprice = floatval($rdt->subtotprice) + floatval($value->price);
                $rdt->subtottax = floatval($rdt->subtottax) + floatval($value->tax);
                $rdt->grandtot = floatval($rdt->grandtot) + floatval($value->amount);
            }
        }
